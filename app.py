...
.metric-card.refugos { border-color: #10b981; }
...
# ============================================================
# 📂 BARRA LATERAL + SELEÇÃO DE COLUNAS
# ============================================================
...
with st.sidebar:
    ...
    f_turno = "Todos"
    if col_turno and not df_temp.empty:
        df_temp["turno_num"] = pd.to_numeric(df_temp[col_turno], errors="coerce")
        turnos_unicos = sorted(df_temp[df_temp["turno_num"] > 0]["turno_num"].dropna().unique().astype(int).astype(str))
...
col_custo = find_column(df, ["custo", "custo refugo"])
...
if f_turno != "Todos" and col_turno:
    df_f = df_f[pd.to_numeric(df_f[col_turno], errors="coerce").astype(int).astype(str) == f_turno]
if f_ano != "Todos":
...
with c1:
    st.markdown(f"""
    <div class="metric-card refugos">
        <div class="metric-label">📦 Total de Refugos</div>
        <div class="metric-value">{total_refugos:,}</div>
    </div>
    """, unsafe_allow_html=True)
...
if "rowid" in df_f.columns and "rowid" not in colunas_validas:
    colunas_validas = ["rowid"] + colunas_validas

df_edit = df_f[colunas_validas].copy()

cfg = {}
if col_apq and col_apq in colunas_validas:
    cfg[col_apq] = st.column_config.SelectboxColumn("APQ", options=["Pendente", "Concluída"], required=True)
if col_qtd and col_qtd in colunas_validas:
    cfg[col_qtd] = st.column_config.NumberColumn("QUANTIDADE", format="%d", min_value=0)
if col_custo and col_custo in colunas_validas:
